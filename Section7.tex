...
\textbf{Static Pressure (hPa):}\nop{PSxC}{}\nop{PSXC}{}\nop{PSx}{}\nop{PSX}{}\nop{PS_A}{}\nop{PSFD}{}\nop{PSFRD}{}\textbf{
}\textbf{\uline{PSx}}\textbf{,\sindex[var]{PSx@\textbf{PSx}}\index{PSx}
}\textbf{\uline{PSxC}}\sindex[var]{PSxC}\index{PSxC}, \textbf{\uline{PS\_A}}\sindex[var]{PS_A@PS\_A}\index{PS_A@PS\_A}\textbf{,
}\textbf{\uline{PSX}}\uline{,}\index{PSX}\sindex[var]{PSX}\textbf{
...
is PSx corrected for local flow-field distortion. (See \href{https://opensky.ucar.edu/islandora/object/archives\%3A8150}{RAF Bulletin \#21}\index{Bulletin 21}
and the discussion in \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/PCOR.pdf}{this memo}
and \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/PcorCalculation.pdf}{this supplement}),
...
the memo \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/PCOR.pdf}{referenced above},
...
\begin{equation}
\frac{\Delta p}{p_{m}}=d_{0}+\frac{q_{m}}{p_{m}}(d_{1}+d_{4}\frac{\alpha^{2}}{a_{r}^{2}})+d_{2}\frac{\alpha}{a_{r}}+d_{3}\thinspace M\label{eq:PCORC130}
\end{equation}
\\
where, for $p_{m}$ = PSFD \sindex[lis]{alpha@$\alpha$=angle of attack},
$q_{m}$ = dynamic pressure (QCF), $\alpha=\mathrm{ATTACK}$ and $a_{r}=1^{\circ}$
(included to keep the equation and coefficients\index{coefficients!sensitivity}
dimensionless), and \{$d_{0},\,d_{1},\,d_{2},\,d_{3},\,d_{4}$\}
= \{-4.389e-03, -2.966e-02, -6.831e-05, 2.672e-02, 2.4466e-03\}. For
PSFRD, the corresponding coefficients are \sindex[lis]{di@$d_{0-2}$=coefficients, pressure correction, C-130}
\{0.007372, 0.12774, $-$6.8776e-4, $-$0.02994, 0.001630\}. The
latter coefficients are significantly different from the coefficients
for PSFD, but the static ports where PSFRD is measured are at a different
location on the fuselage so different flow-distortion effects are
expected.
\end{quote}
%
\end{minipage}\\
...
\begin{minipage}[t]{1\columnwidth}%
\begin{quote}
\textbf{For the GV,}\footnote{See \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/PCOR.pdf}{this memo}
and \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/PcorCalculation.pdf}{this revision}for
details regarding implementation of this representation of $\Delta p$
for the GV: }\\
\begin{equation}
...
measurements of $p$ and $q$, using dry-air values for $R$, $c_{v}$
abd $c_{p}$, via\\
\begin{equation}
M=\left\{ \left(\frac{2c_{v}}{R}\right)\left[\left(\frac{p+q}{p}\right)^{R/c_{p}}-1\right]\right\} ^{1/2}\,\,\,.\label{eq:MachEquation}
\end{equation}
For additional information on these correction coefficients, see \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/ARISTO-LAMS.pdf}{this note}
and Cooper et al.~(2014)\footnote{\href{http://www.atmos-meas-tech.net/7/3215/2014/}{Atmos. Meas. Tech., 7, 3215-3231},
2014 doi:10.5194/amt-7-3215-2014}.\\
\\

\textbf{Dynamic Pressure (hPa):}\nop{QCx}{}\nop{QCxC}{}\nop{QCX}{}\nop{QCXC}{}\textbf{
}\textbf{\uline{QCx}}\textbf{, }\textbf{\uline{QCxC}}\textbf{,
}\textbf{\uline{QCX}}\textbf{, }\textbf{\uline{QCXC}}\index{QCXC}\index{QCx}\index{QCxC}\sindex[var]{QCx}\sindex[var]{QCxC}\sindex[var]{QCXC}\index{QCX}\sindex[var]{QCX}\\
...
the notes referenced in the preceding section, and also \href{https://opensky.ucar.edu/islandora/object/archives\%3A8150}{RAF Bulletin 21}\index{Bulletin 21}
...
\\
In the case of QCRC\index{QCRC!flow-angle correction} from the GV,
one additional correction is applied (beginning 2017). The uncorrected
measurement QCR is affected by flow angles, while QCF is not (for
modest angle of attack or sideslip), so an additional adjustment is
needed. The needed correction can be found by using an empirical relationship
matching QCR to QCF, which leads to the following equation: \\
\[
\mathrm{\{QCRC\}}=b_{0}+b_{1}\mathrm{\{QCR\}}+b_{2}\mathrm{\{AKRD\}^{2}}+b_{3}\mathrm{\{SSRD\}^{2}-\Delta p}
\]
where $\Delta p$ is given by (\ref{eq:PCORforGV}) and the coefficients
are \{$b_{0-3}$\}=\{$-0.5635$, $0.9982$, , 0.0273, $0.0562$\}.
Some justification for this correction is contained in \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/Recommendation161107b.pdf}{this note.}
A similar correction is not made for measurements from the C-130 radome
because they do not appear to be necessary, as discussed in that note.\\
 

\textbf{D-Value (m):}\nop{DVALUE}{}\textbf{ }\textbf{\uline{DVALUE}}\textbf{\index{DVALUE}}\sindex[var]{DVALUE}\\
...
is calculated from \{GEOPHT\}$-$\{PALT\} and, for appropriate flight
segments, can be used to measure height gradients on a constant-pressure
surface. Prior to 2018, this was calculated from \{GGALT\} -- \{PALT\}.

\textbf{Special Pressure Measurements (hPa):}\nop{PSDPx}{}\nop{CAVP_x}{}\nop{PCAB}{}\nop{PSURF}{}\textbf{
}\textbf{\uline{PSDPx}}\textbf{, }\textbf{\uline{CAVP\_x}}\textbf{,
}\textbf{\uline{PCAB}}\textbf{, }\textbf{\uline{PSURF}}\index{PCAB}\index{PSDPx}\index{CAVP_x@CAVP\_x}\index{PSURF}\sindex[var]{PSDPx}\sindex[var]{CAVP_x@CAVP\_x}\sindex[var]{PCAB}\sindex[var]{PSURF}\index{pressure!cabin}\index{pressure!dew-point cavity}\index{pressure!surface}\\
...
RTHRx\sindex[var]{RTHRx}}\index{RTHRx}, \textbf{\uline{RTX}}\index{RTX}\sindex[var]{RTX}\\
...
\textbf{Ambient Temperature ($^{\circ}$C):}\nop{ATx}{}\nop{ATX}{}\nop{ATxJ}{}\nop{ATxD}{}\textbf{
}\textbf{\uline{ATx}}\textbf{\sindex[var]{ATx}\index{ATx}, }\textbf{\uline{ATX}},\index{ATX}\sindex[var]{ATX}\textbf{
}\textbf{\uline{ATxH}}\sindex[var]{ATxH}\index{ATxH}\textbf{,
}\textbf{\uline{ATxD}}\index{ATxD}\index{ATxD}\\
...
\end{equation}
Some studies of the recovery factor are discussed further in \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/RecoveryFactorStudy.pdf}{this memo}.\\
\\
...
\textbf{In-cloud Air Temperature, Radiometric ($^{\circ}$C):}\nop{AT_ITR}{}
\textbf{\uline{AT\_ITR}}\index{AT_ITR@AT\_ITR}\sindex[var]{AT_ITR@AT\_ITR}
\\
...
\textbf{Ophir Air Temperature ($^{\circ}$C):}\nop{OAT}{}\textbf{
}\textbf{\uline{OAT}}\index{OAT}\sindex[var]{OAT}\\
...
\begin{hangparagraphs}
\textbf{Dew/Frost Point ($\text{�}$C):}\nop{DPx}{}\nop{DP_x}{}\nop{MIRRTMP_DPX}{}\textbf{
}\textbf{\uline{DPx}}\sindex[var]{DPx}\index{DPx}\textbf{, }\textbf{\uline{DP\_x}}\index{DP_x@DP\_x}\sindex[var]{DP_x@DP\_x}\textbf{,
...
\textbf{Corrected Dew Point (C):}\nop{DPxC}{}\nop{DPXC}{}\textbf{
}\textbf{\uline{DPXC}}\textbf{, }\textbf{\uline{DPxC}}\sindex[var]{DPxC}\index{DPxC}\index{DPXC}\sindex[var]{DPXC}\textbf{}\footnote{See also DP\_VXL and DP\_CR2C below}\textbf{}\\
...
for more information, see \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/VaporPressureV3.pdf}{this memo}.\\
\\
...
temperature is based on the Murphy and Koop\footnote{Q. J. R. Meteorol. Soc. (2005), 131, pp. 1539--1565}\index{Murphy and Koop, 2005}\index{pressure!water vapor!equilibrium}
...
($^{\circ}$C):}\nop{FP_CR2}{}\nop{MIRRORT_CR2}{}\textbf{ }\textbf{\uline{FP\_CR2\sindex[var]{FP_CR2@FP\_CR2}\index{FP_CR2@FP\_CR2}}}\textbf{,}\\
...
($^{\circ}$C):}\nop{DP_CR2C}{}\textbf{ }\textbf{\uline{DP\_CR2C}}\index{DP_CR2C@DP\_CR2C}\sindex[var]{DP_CR2C@DP\_CR2C}\\
...
(molecules cm$^{-3}$):}\nop{RAWCONC_VXL}{}\textbf{ }\textbf{\uline{RAWCONC\_VXL}}\index{RAWCONC_VXL@RAWCONC\_VXL}\sindex[var]{RAWCONC_VXL@RAWCONC\_VXL}
\emph{}\\
...
(molecules cm$^{-3}$):}\nop{CONCV_VXL}{}\textbf{ }\textbf{\uline{CONCV\_VXL}}\uline{:}\index{CONCV_VXL@CONCV\_VXL}\sindex[var]{CONCV_VXL@CONCV\_VXL}\\
...
\textbf{Voltage Output from the UV Hygrometer (V):}\nop{XSIGV_UHV}{}\textbf{
}\textbf{\uline{XSIGV\_UVH}}\textbf{\index{hygrometer!UV}\index{XSIGV_UVH@XSIGV\_UVH}}\sindex[var]{XSIG_UVH@XSIG\_UVH}\\
...
cm$^{-3}$):}\nop{CONCH_UVH}{}\textbf{ }\textbf{\uline{CONCH\_UVH}}\index{CONCH_UVH@CONCH\_UVH}\sindex[var]{CONCH_UVH@CONCH\_UVH}\\
...
\textbf{Water Vapor Pressure (hPa):}\nop{EWx}{}\nop{EWX}{}\nop{EW_UHV}{}\nop{EDPC}{}\textbf{
...
\textbf{Relative Humidity (per cent or Pa/hPa):}\nop{RHUM}{}\textbf{
}\textbf{\uline{RHUM}}\sindex[var]{RHUM}\index{RHUM}\\
...
\textbf{Absolute Humidity, Water Vapor Density (g/m$^{3}$):}\nop{RHOx}{}\textbf{
}\textbf{\uline{RHOx}}\sindex[var]{RHOx}\index{RHOx}\\
...
be documented here because the instruments are obsolete. See \href{https://opensky.ucar.edu/islandora/object/archives\%3A8729}{RAF Bulletin 9}
for the processing previously used for archived measurements from
the Lyman-alpha hygrometers.\\

\textbf{Specific Humidity (g/kg):}\nop{SPHUM}{}\textbf{ }\textbf{\uline{SPHUM}}\sindex[var]{SPHUM}\index{SPHUM}\\
...
\textbf{Mixing Ratio (g/kg):}\nop{MR}{}\nop{MRCR}{}\nop{MRLA}{}\nop{MRLH}{}\nop{MRVCL}{}\textbf{
}\textbf{\uline{\label{Mixing-Ratio-(g/kg):}MR}}\sindex[var]{MR}\textbf{\index{MR},
...
\begin{hangparagraphs}
\textbf{Potential Temperature (K):}\nop{THETA}{}\textbf{ }\textbf{\uline{THETA}}\sindex[var]{THETA}\index{THETA}\\
...
\textbf{Pseudo-Adiabatic Equivalent Potential Temperature (K):}\nop{THETAP}{}\nop{THETAE}{}\textbf{
}\textbf{\uline{THETAP}}\sindex[var]{THETAP}\sindex[var]{THETAE}\index{THETAE}\index{THETAE}\\
...
\emph{Mon. Wea. Review, }\textbf{137, }3137--3148.} This is discussed in the memo available at \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/ThetaE.pdf}{this link}.
...
\emph{Mon. Wea. Rev.,} \textbf{108,} 1046--1053. } is used to determine $T_{L}$:\\
\begin{equation}
...
ATX\index{ATX} = ambient temperature ($^{\circ}C$)\\
EDPC\index{EDPC} = water vapor pressure (hPa) -- now superceded
by EWX\\
...
\textbf{Virtual Temperature ($\text{�}$C):}\nop{TVIR}{}\textbf{
}\textbf{\uline{\label{TVIR}TVIR}}\sindex[var]{TVIR}\index{TVIR}\\
...
\textbf{Virtual Potential Temperature (K):}\nop{THETAV}{}\textbf{
}\textbf{\uline{THETAV}}\sindex[var]{THETAV}\index{THETAV}\\
...
conditions. For further information, see \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/VirtualTemperature.pdf}{this note}.\\
\fbox{\begin{minipage}[t]{0.95\columnwidth}%
...
\end{equation}
 %
\end{minipage}}\href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/VirtualTemperature.pdf}{this note}\\

\textbf{Wet-Equivalent Potential Temperature (K):}\nop{THETAQ}{}\textbf{
}\textbf{\uline{THETAQ}}\index{THETAQ}\sindex[var]{THETAQ}\\
\emph{The absolute temperature reached if a parcel of air were to
...
of liquid water, and other symbols are as used previously. See \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/ThetaE.pdf}{this memo}
