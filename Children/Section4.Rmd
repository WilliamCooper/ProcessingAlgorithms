...
(\#eq:PCORforGV)
\end{equation}  
where, for $p=\{PSF\}$, $q=\{QCF\}$. $\alpha=\{ATTACK\}$, $a_r=1^\circ$ (included to keep the equation and coefficients dimensionless), and $\{a_{0-3}\}$={$-0.012255$, $0.075372$, $−0.087508$, $0.002148$}.^[[This memo](www/PCOR.pdf) and [this revision](www/PcorCalculation.pdf) provide more details regarding implementation of this representation of Δp for the GV.]<span id="punch4.10" label="punch4.10">`r ifelse(PToDo,"[ToDo4.10]","")`</span></p>  
...
A Rosemount Model 1221 differential pressure transducer is used for current measurements of dynamic pressure on the C-130, and a Honeywell PPT transducer is used on the GV.<span id="punch4.1" label="punch4.1">`r ifelse(PToDo,"[ToDo4.1]","")`</span> This measurement enters the calculation of true airspeed and Mach number and so is needed to calculate many derived variables.  
...
Because the values used in processing have varied, the project reports should be consulted to find what was used for particular projects. The Goodrich Technical Report 5755<span id="punch4.4" label="punch4.4">`r ifelse(PToDo,"[ToDo4.4]","")`]</span> documents wind-tunnel testing of the probes formerly made by Rosemount. Their plot showed that, for heated sensors, there is a significant variation with Mach number ($M$); cf their Eq.\ (38). The dependence in their plot is represented well by the following equations, where $\alpha_r^{[h]}$ refers to heated probes and $\alpha_r^{[u]}$ to unheated probes:  
\begin{equation}
...
(\#eq:housingCorr)
\end{equation}
<span id="punch4.6" label="punch4.6">`r ifelse(PToDo,"[ToDo4.6]","")`</span>
...
*The mirror temperature in the CR2 cryogenic hygrometer,* which is normally the frost point inside the measuring chamber of the instrument*.* <span id="punch4.7" label="punch4.7">`r ifelse(PToDo,"[ToDo4.7]","")`</span>The measurement is often suspect when the value is above about $-15^\circ\mathrm{C}$; the measurement is intended for use below this value. The CR2 is a cabin-mounted instrument, so the measured pressure (P_CR2) in the instrument must be used with the ambient pressure (PSXC) to convert the measurement to ambient humidity measures like DP_CR2 and EW_CR2.  
...
<span id="punch4.12" label="punch4.12">`r ifelse(PToDo,"[ToDo4.12]","")`</span><br />
...
*The component of the wind in the vertical direction.* This is the standard calculation of vertical wind, obtained from the difference between the measured vertical component of the relative wind and the vertical motion of the aircraft (usually GGVSPD in recent projects).<span id="punch4.13" label="punch4.13">`r ifelse(PToDo,"[ToDo4.13]","")`</span> This should be used in preference to WI even if the latter is present; see the discussion of WP3 in Section \@ref(inertial-reference-systems). Positive values are toward the zenith. Sometimes WIY, another calculation of vertical wind based on AKY instead of AKRD, is included, and WIX may indicate (in its attributes) which of WIC or WIY is thought to be better and will equal that preferred variable.  
