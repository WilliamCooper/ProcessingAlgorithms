...
The instruments used to collect the measurements that lead to the variables in this section are described on the EOL web site, in the “State Parameters” section at [this URL](https://www.eol.ucar.edu/aircraft-instrumentation). The data acquisition and processing for these variables and the calibration coefficients used where applicable are described there.  
...
*The atmospheric pressure at the flight level of the aircraft, measured by a calibrated absolute (barometric) transducer at location x.* PSx is the measured static or ambient pressure before correction, and it may be affected by local flow-field distortion. PS_A is the pressure measurement taken from the avionics system on the aircraft, processed via unknown algorithms in the avionics system that may smooth, correct, and perhaps delay the result. PSxC is PSx corrected for local flow-field distortion (see [RAF Bulletin #21](https://opensky.ucar.edu/islandora/object/archives%3A8150) and the discussion in [this memo](www/PCOR.pdf) and [this supplement](www/PcorCalculation.pdf)); PSXC is the preferred corrected measurement used for derived calculations. These measurements have been made using various sensors, so it is best to consult the project documentation for the transducer used. Recent measurements from both the C-130 and the GV have been made using a Paroscientific Model 1000 Digiquartz Transducer.  

Corrections to the pressures have been determined by reference to some standard, including a “trailing cone” sensor, the pressure PS_A from the cockpit avionics system, or (since 2012) the Laser Air Motion Sensing System (LAMS). The latter correction is discussed in the memo [referenced above](www/PCOR.pdf), where corrections used prior to 2011 are also discussed. Beginning in 2012, the deduced corrections *Δp* to the measured pressures as functions of dynamic pressure *q*, angle of attack *α*,^[A weakness is this form for the pressure correction is that occasionally the radome ports become plugged with ice and the measurement of angle of attack is not available. When the variable ATTACK representing angle of attack is invalid, the angle of attack is instead calculated from PITCH = −VSPD / TASX, which approximates the angle of attack if the vertical wind is zero.] and the Mach number *M* are described by the following equations and coefficients:  
```{r child='css.Rmd'}
```
>**For the C-130:**^[For C-130 measurements prior to 2012 but after September 2003, the correction applied to PSF was *Δp* = *p* + max ((3.29 + {*QCX*} $\times$ 0.0273), 4.7915) using units of hPa. Prior to Sept 2003, the correction was *Δp* = max ((4.66 + 11.4405*Δp~α~*/*Δq~r~*, 1.113) where *Δp~α~* is the pressure difference between vertically aligned ports on the radome and *Δq~r~* is the dynamic pressure from the radome. For both PSFD and PSFRD, the correction prior to (2012) was *Δp* = *p* + max ((3.29 + {*QCX*} $\times$ 0.0273), 4.7915). For GV measurements Aug 2006 to 2012, *Δp* = (-1.02 + 0.1565 $\times$ q) + q1 $\times$ (0.008 + q1 $\times$ (7.1979e-09 $\times$ q1 - 1.4072e-05). Before Aug 2006: *Δp* = (3.08 - 0.0894 $\times$ {PSF}) + {QCF} $\times$ (-0.007474 + {QCF} $\times$  4.0161e-06).]    
\begin{equation}
\frac{\Delta p}{p_{m}}=d_{0}+\frac{q_{m}}{p_{m}}(d_{1}+d_{4}\frac{\alpha^{2}}{a_{r}^{2}})+d_{2}\frac{\alpha}{a_{r}}+d_{3}\thinspace M
(\#eq:PCORC130)
\end{equation}
where, for $p_m=\{PSFD\}$, $q_m$ = dynamic pressure {QCF}, $\alpha=\{ATTACK\}$, $a_r=1^\circ$ (included to keep the equation and coefficient dimensionless), and $\{d_{0-4}\}$={-4.389e-03, -2.966e-02, -6.831e-05, 2.672e-02, 2.4466e-03}. For PSFRD, the corresponding coefficients are {0.007372, 0.12774, $-6.8776\times 10^{-4}$, $-0.02994$, 0.001630}. The latter coefficients are significantly different from the coefficients for PSFD, but the static ports where PSFRD is measured are at a different location on the fuselage so different flow-distortion effects are expected.  
 
>**For the GV:**^[[This memo](www/PCOR.pdf) and [this revision](www/PcorCalculation.pdf) provide more details regarding implementation of this representation of Δp for the GV.]  
\begin{equation}
\frac{\Delta p}{p}=a_{0}+a_{1}\frac{q}{p}+a_{2}M^{3}+a_{3}\frac{\alpha}{a_{r}}
(\#eq:PCORforGV)
\end{equation}  
where, for $p=\{PSF\}$, $q=\{QCF\}$. $\alpha=\{ATTACK\}$, $a_r=1^\circ$ (included to keep the equation and coefficients dimensionless), and $\{a_{0-3}\}$={$-0.012255$, $0.075372$, $−0.087508$, $0.002148$}.`r ToDoLink('4.10')`</p>  
  
<br />
In equations \@ref(eq:PCORC130) and \@ref(eq:PCORforGV) the Mach number is calculated from the uncorrected measurements of $p$ and $q$, using dry-air values for $R$, $c_v$ and $c_p$, via  
\begin{equation}
M=\left\{ \left(\frac{2c_{v}}{R}\right)\left[\left(\frac{p+q}{p}\right)^{R/c_{p}}-1\right]\right\}^{1/2}\,\,\,.
(\#eq:MachEquation)
\end{equation}  

The pressure PSTF is measured at the static-pressure port of a pitot-static tube mounted on the top of the fuselage. The correction to this pressure (leading to PSTFC) differs from the pressure corrections listed above in that it is based on an empirical fit to other measurements of static pressure. The airflow at that sensor is distorted by the fuselage, so that measurement is usually less reliable than other measures of static pressure. See [this note](www/WindInSOCRATES.pdf) for additional discussion of this measurement.  

For additional information on these correction coefficients, see [this note](www/ARISTO-LAMS.pdf) and Cooper et al. (2014).^[<a href="http://www.atmos-meas-tech.net/7/3215/2014/">Atmos. Meas. Tech., 7, 3215-3231</a>, 2014 doi:10.5194/amt-7-3215-2014.]  
  
#### Total Pressure (hPa): PT_A {-#ptot}  

*The total pressure (dynamic plus static)* provided by an aircraft avionics system. An alternate measure of the total pressure can be calculated from PSF + QCF or other combinations of static and dynamic pressure as measured by the research instruments.  
...
\@ref(eq:PCORforGV) except that the correction applied to $q$ is $-\Delta p$ with reversed sign because $q=p_t-p_a$ and the correction term arises primarily from the correction to $p_a$. The same correction is applied to QCR because it is also measured relative to the static pressure ports so errors in the pressure sensed at those ports affect QCR in the same way that QCF is affected.`r ToDoLink('4.3')` See the notes referenced in the preceding section, and also [RAF Bulletin 21](https://opensky.ucar.edu/islandora/object/archives%3A8150) for the corrections applied to earlier data files.[^fnlabelspecial]
...
A Rosemount Model 1221 differential pressure transducer is used for current measurements of dynamic pressure on the C-130, and a Honeywell PPT transducer is used on the GV.`r ToDoLink('4.1')` This measurement enters the calculation of true airspeed and Mach number and so is needed to calculate many derived variables.  
...
where $\Delta p$ is given by \@ref(eq:PCORforGV) and the coefficients are {$b_{0-3}$={$−0.5635$, $0.9982$, $0.0273$, $0.0562$}.  Some justification for this correction is contained in [this note](www/Recommendation161107b.pdf). A similar correction is not made for measurements from the C-130 radome because they do not appear to be necessary, as discussed in that note.    

The measurement QCTF is made at the dynamic-pressure port of a pitot-static sensor mounted on the top of the fuselage. Because this is located in a region of airflow distortion around the fuselage of the GV, special processing is required. The method proposed in [this note](www/WindInSOCRATES.pdf) is to calculate a corrected dynamic pressure from QCTFC=QCTF+PSTF-PSXC. For high-rate files, PSXC is low-pass-filtered with a cut-off frequency of 0.5 Hz to eliminate high-frequency noise in this measurement.  
...
*PSDPx and CAVP_x are measurements of the pressure in the housing of the dew-point sensors,* as discussed in connection with DPxC. *PCAB is a measurement of the pressure in the cabin of the aircraft. PSURF is the estimated surface pressure* calculated from HGME (a radar-altimeter measurement of height), TVIR, PSXC,`r ToDoLink('4.11')` and MR using the thickness equation. [TVIR](#TVIR) and [MR](#MR) are described later in this section, and [HGME](#HGME) was described in Section \@ref(other-measurements-of-aircraft-altitude). The average temperature for the layer is obtained by using HGME and assuming a dry-adiabatic lapse rate from the flight level to the surface. Because of this assumption, the result is only valid for flight in a well-mixed surface layer or in other conditions in which the temperature lapse rate matches the dry-adiabatic lapse rate.^[The symbol ^†^ indicates that values are included in the table of constants in Sect.\ \@ref(constants-and-symbols).]  
...
#### Recovery Temperature (ºC): RTx, RTxH, RTHRx, RTX {-#recovery-t}
...
For Rosemount or HARCO temperature probes in current use, the recovery temperature is near the total temperature, but all probes must be corrected to obtain either true total temperature or true ambient temperature. In the standard output, the variable name also conveys the sensor type: RTF\# or RTx is a measurement from a Rosemount Model 102 non-deiced temperature sensor, RT\#H or RTxH is the measurement from a Rosemount Model 102 anti-iced (heated) temperature sensor, and RTH\# or RTHx is the measurement from a HARCO heated sensor. The aircraft avionics system also provides a measured recovery temperature, named RT_A. Some past experiments also used a reverse-flow temperature housing and a fast-response “K” housing; the associated variable names for these probes were TTRF and TTKP.^[See the related obsolete variables TTx, which are previously used names for these variables. The names were changed to clarify that the quantity represented is the recovery temperature, not the total temperature.]  

#### Ambient Temperature (^∘^C): ATx, ATX, ATxH {-#ambient-t}

*The temperature of the atmosphere at the location of the aircraft, as it would be measured by a sensor at rest relative to the air.*    

The ’x’ in the name of the variable used for ambient temperature, ATx, conveys the same information regarding sensor type and location as the variable name used with total (recovery) temperature. See the discussion above regarding RTx. Variable names that appear in project archives include ATF, ATF1, AFT2, ATH, ATH1, ATH2, ATH3, ATH4, ATHL1, ATHL2, ATFH1, ATFH2, ATFR, ATRL, and ATRR. The VCSEL hygrometer incorporates a temperature sensor (AT_VXL or AT_VXL2), with measurements recorded in units of kelvin. The measurements of ambient temperature (also known as the static air temperature), except AT_VXL and AT_VXL2, are calculated from measured recovery temperatures, which are increased above the ambient temperatures by dynamic heating caused by the airspeed of the aircraft. The calculated temperature therefore depends on the recovery temperature RTx as well as the dynamic and ambient pressure, usually respectively QCXC and PSXC. The ambient and dynamic pressures are first corrected from the raw measurements QCX and PSX to obtain variables that account for deviations caused by airflow around the aircraft and/or position-dependent systematic errors, as discussed in the section describing PSxC. In addition, the aircraft avionics provides estimates of air temperature (AT_A and AT_A2).  

The following basic equations are developed on the basis of conservation of energy for a perfect gas undergoing an adiabatic compression. This section combines discussion of the calculations of temperature and <a name=airspeed1>airspeed</a>, to reflect the linkage between these derived measurements. To provide accuracy in the equations, this discussion considers effects of the humidity of the air on characteristics like the gas constant and the specific heats. Most archived data before 2012 used values for dry air, although a special variable TASHC has been used to represent the true airspeed in cases where the humidity correction was significant. That variable was based on a good approximation to the results from the following equations; see the discussion of TASHC later in this section. TASHC is now considered an obsolete variable. New variables ATxD and TASDRY have been introduced that neglect the humidity corrections and perform all calculations as if the humidity were negligible^[ATxD has not appeared yet, as of 2022, in data archives.].    

As discussed above, temperature sensors on aircraft that are exposed to the airflow do not measure the total temperature but rather the temperature of the air immediately in contact with the sensing element. This air will not have undergone an adiabatic deceleration completely to zero velocity relative to the sensor and hence will have a temperature $T_r$ somewhat less than the total temperature $T_t$ that would require the air to reach zero velocity relative to the sensor. $T_r$ is the measured or “recovery” temperature. The ratio of the actual temperature difference attained to the temperature difference relative to the total temperature is defined to be the “recovery factor” $\alpha_r$:
\begin{equation}
...
(\#eq:Ua)
\end{equation}
where primes on quantities like $c_p^\prime$, or (below) $c_v^\prime$ and $R^\prime$ denote properties of moist air, respectively the specific heat at constant pressure, specific heat at constant volume, and gas constant.  

>**Moist-Air Considerations**<br>
...
et al.\ 1999](https://journals.ametsoc.org/view/journals/atot/16/7/1520-0426_1999_016_0860_iwmora_2_0_co_2.xml):  
\begin{equation}
...
In \@ref(eq:Ua) $U_a$, $U_r$ and $U_t$ are respectively the aircraft true airspeed, the airspeed relative to the aircraft of the air in thermal contact with the sensor, and the airspeed of air relative to the aircraft when fully brought to the motion of the sensor (i.e., zero). Then, from \@ref(eq:Ua)  \begin{equation}
...
(\#eq:alphau)
\end{equation}  
Some studies of the recovery factor are discussed further in [this memo](www/RecoveryFactorStudy2021.pdf).  
...
>The relationship can be derived by first calculating the dimensionless Mach number $M$, which is the ratio of the <a name=airspeed3>airspeed</a> to the speed of sound $U_{s}=\sqrt{\gamma^{\prime}R^{\prime}T_{a}}$, where $\gamma^\prime$ is the ratio of specific heats of (moist) air, $c_p^\prime /c_v^\prime$ and $R^\prime$ is the gas constant for moist air. The Mach number is a function of air temperature only and can be determined as follows:  
>
...
(2005) equations^[Prior to 2010, the vapor pressure relationship used was the Goff-Gratch formula as given in the Smithsonian Tables (List, 1980). For $T_s=373.16$ K and $T_k=T+T_0$ where $T$ is the dewpoint temperature in $^\circ\mathrm{C}$ and $T_0=273.15\,K$,
the equation used to calculate the vaper pressure $e_w$ in hPa was this:\
$$log_{10}\frac{e_w}{a_0}=a_1(\frac{T_s}{T_k}-1)+a_2log_{10}(\frac{T_s}{T_k})+a_3(10^{a_4(1-\frac{T_k}{T_s})-1})+ a_5(10^{a_6(\frac{T_s}{T_k}-1)}-1)$$
where $\{a_{0-6}\}=\{1013.246, -7.90298, 5.02808, -1.3816\times 10^{-7}, 11.334, 8.1328\times 10^{-3}, -3.49149\}$. The coefficient $a_4$ should have been 11.344, but this error only caused $e_w$ to be less then 0.005% too high for dewpoints above $-40^\circ\mathrm{C}$. For vapor pressure relative to ice, the equation was:
$$log_{10}\frac{e_i}{b_0}=b_1(\frac{T_3}{T_k}-1)+b_2log_{10}(\frac{T_3}{T_k})+b_3(1-\frac{T_k}{T_3})$$
where $T_k$ is the frost point in kelvin, $T_3=273.16\,K$ is the triple point of water, and the coefficients are $\{b_{0-3}\}=\{6.1071, -9.09718, -3.56654, 0.876793\}$. $T_3$ was erroneously set to 273.15\ K, but that introduced negligible error.
<!-- $$e_w = 10^{(-7.90298 * (\frac{T_{DP}{T_0} - 1.0)+5.02808 * log10(T_{DP} / T_0)$$ -->
<!--     & -1.3816e-7 * (10^{11.334 * (1.0 - T_0 / T_{DP})} - 1.0)\notag\\ -->
<!--     & +8.1328e-3 * (10^{-3.49149 * (T_{DP} / T_0 - 1.0)} - 1.0) + log10(1013.246))} -->
<!-- (\#eq:GoffGratch) -->
<!-- \end{align} -->
].
...
\end{equation}
\begin{equation}
e_{s,w}(T_{DP})=c_{0}\exp\left((\alpha-1)c_{6}+d_{2}(\frac{T_{0}-T_{DP}}{T_{DP}T_{0}})+d_{3}\ln(\frac{T_{DP}}{T_{0}})+d_{4}(T_{DP}-T_{0})\right)
(\#eq:MKw)
\end{equation}
\begin{equation}
...
The coefficients used in the above formulas are given in the following tables, with the additional definitions that $\alpha_T=\tanh(c_5(T_{DP}-T_x))$, $T_x=218.8$\ K, and $d_i=c_i+\alpha_Tc_{i+5}$ for i = {2,3,4}:

<p><center>
```{r MKcoef, echo = FALSE}
MKC <- data.frame(coefficient=c('$b_0^\\prime$', '$b_1$', '$b_2$', '$b_3$',
                                '$f_1$', '$f_2$', '$f_3$'))
MKC$value <- c('$6.111536$\ hPa', '$-5723.265$\ K', '$3.53068$',
...
```{r child='DPbox.Rmd'}
```
For other instruments that measure water vapor density, such as a Lyman-alpha or the UV hygrometer or the tunable diode laser hygrometers (including the Vertical Cavity Surface Emitting Laser (VCSEL) hygrometer), a similar conversion is made from vapor density to dew point, as described in the next paragraph.  

#### Dew Point Determined from the VCSEL or UV Hygrometer (^∘^C): DP_VXL, DP_UVH {-#vcsel-dp}

*The dew point temperature* determined from the measured water vapor density from the VCSEL or UV hygrometer. The measured water vapor density is converted from a molecular density [molecules$\,$cm$^{-3}$] to a water-vapor pressure as described by \@ref(eq:VCSELew) included with the discussion of the variable [EW_VXL](#ewx). Then 
DP_VXL is given by DPxC from \@ref(eq:DPxC2). See also [CONCV_VXL](#vcsel-corr) and [CONCH_UV](#uvh-n) below.  

#### Frost Point Temperature from the CR2 Cryogenic Hygrometer (^∘^C): FP_CR2, MIRRORT_CR2 {-#mirror-cr2}

*The mirror temperature in the CR2 cryogenic hygrometer,* which is normally the frost point inside the measuring chamber of the instrument*.* `r ToDoLink('4.7')`The measurement is often suspect when the value is above about $-15^\circ\mathrm{C}$; the measurement is intended for use below this value. The CR2 is a cabin-mounted instrument, so the measured pressure (P_CR2) in the instrument must be used with the ambient pressure (PSXC) to convert the measurement to ambient humidity measures like DP_CR2 and EW_CR2.  
#### Corrected Dew Point Temperature from the CR2 Cryogenic Hygrometer (^∘^C): DP_CR2C {-#dp-cr2}
...
#### Uncorrected Water Vapor Number Density from the VCSEL Hygrometer (molecules cm^-3^):  RAWCONC_VXL {-#vcsel-uncor}
...
#### Corrected Water Vapor Concentration from the VCSEL Hygrometer (molecules cm^-3^): CONCV_VXL, CONC_H2O_VXL, CONCV_VXL2 {-#vcsel-corr}

*The corrected water vapor number density produced by the VCSEL hygrometer,* after minor corrections for ambient temperature, pressure, laser intensity and water vapor concentration.`r ToDoLink('4.8')` For more information on calibration and data processing for this instrument, see the [instrument web page](https://www.eol.ucar.edu/instruments/vertical-cavity-surface-emitting-laser-vcsel-hygrometer) and additional documentation there. A new version, VCSEL2, produces CONCV_VXL2. The variable name CONC_H2O_VXL has not been used since 2011.  
...
The formula for obtaining the ambient water vapor pressure as a function of dew point is given in the discussion of DPxC above, Eqs.\ \@ref(eq:MKw) and \@ref(eq:enhance), where the calculations leading to EWx and EWX are also discussed. EWX (or previously EDPC) is the preferred variable that is selected from among the possibilities {EWx} for subsequent calculation of derived variables.  

For the VCSEL hygrometer, EW_VXL is determined from CONCV_VXL as follows:
\begin{equation}
\{\mathrm{EW\_VXL}\} = C^\prime k\,\{\mathrm{CONCV\_VXL}\}\times (\{\mathrm{ATX}\}+273.15)
(\#eq:VCSELew)
\end{equation}
where $k$ is the Boltzmann constant and $C^\prime=(\mathrm{100cm/m})^3(\mathrm{hPa/100Pa})=10^4$ converts units to hPa.  
...
#### Relative Humidity (percent or Pa/hPa): RHUM {-#rhumw}

*The ratio of the water vapor pressure to the water vapor pressure in equilibrium over a plane* **liquid** *-water surface,* scaled to express the result in units of percent or Pa/hPa:   
```{r child='RHUMbox.Rmd'}
```
To follow normal conventions, the change in equilibrium vapor pressure that arises from the enhancement factor is not included in the calculated relative humidity, even though the true relative humidity should include the enhancement factor as specified in \@ref(eq:enhance) in the denominator of \@ref(eq:RHUM).  

#### Relative Humidity with respect to Ice (percent or Pa/hPa): RHUMI {-#rhumi}

*The ratio of the water vapor pressure to the water vapor pressure in equilibrium over a plane* **ice**  *surface,* scaled to express the result in units of percent or Pa/hPa:  
```{r child='RHIbox.Rmd'}
```

To follow normal conventions, the change in equilibrium vapor pressure that arises from the enhancement factor is not included in the calculated relative humidity, even though the true relative humidity should include the enhancement factor as specified in \@ref(eq:enhance) in the denominator of \@ref(eq:RHUMI).  

#### Absolute Humidity, Water Vapor Density (g m^-3^): RHOx, RHODT {-#rho}

*The water vapor density* computed from various measurements of humidity as indicated by the ’x’ suffix, and conventionally expressed in units of g m^-3^. The calculation proceeds in different ways for different sensors. For sensors that measure a chilled-mirror temperature, the calculation is based on the equation of state for a perfect gas and uses the water vapor pressure determined by the instrument, as in the following box:`r ToDoLink('4.9')`  

```{r child='RHObox.Rmd'}
```
For the Lyman-alpha probes and the newer Lyman-alpha version called the UV hygrometer, the basic measurement from the instrument is the water vapor density, **RHOUV** or **RHOLA,** determined by applying calibration coefficients to the measured signals (XUVI or VLA). In addition, a slow update to a dew-point measurement is used to compensate for drift in the calibration. The algorithm for the UV Hygrometer is as described in the following box; the processing used for early projects with the Lyman-alpha instruments is similar but more involved and won’t be documented here because the instruments are obsolete. See [RAF Bulletin 9](https://opensky.ucar.edu/islandora/object/archives%3A8729) for the processing previously used for archived measurements from the Lyman-alpha hygrometers.  

<p class=box>  
This algorithm for RHOUV is missing.
</p><p class=box2><p>

For the VCSEL hygrometer, the original measurement is the water-vapor molecular density, corrected to ambient temperature. An absolute-humidity variable is not normally produced, but RHODT may be derived from EW_VXL if EWX=EW_VXL. The variable that otherwise would be RHO_VXL can be calculated using EW_VXL for the water-vapor pressure in \@ref(eq:RHOx).  
...
#### Mixing Ratio (g/kg): MR, MRCR, MRLA, MRLA1, MRLH, MRVXL, MRUV  {-#MR}

*The ratio of the mass of water to the mass of dry air in the same volume of air,* conventionally expressed in units of g/kg or per mille. Mixing ratios may be calculated for the various instruments measuring humidity on the aircraft, and the variable names reflect the source: MR from the dewpoint hygrometers, MRCR from the cryogenic hygrometer, MRLA from the Lyman-alpha sensor, MRLA1 if there is a second Lyman-alpha sensor, MRLH from a tunable-diode laser hygrometer, MRVXL from the VCSEL hygrometer (also a laser hygrometer), and MRUV from the UV hygrometer. The example in the box below is for the case of the dewpoint hygrometers; others are analogous.<br /></p>
```{r child='MRbox.Rmd'}
```

#### Volume Mixing Ratio (ppmv): VMR_VXL, VMR_VXL2 {-#vmr}  

*The mixing ratio by volume,* or the ratio of molecules of water vapor to the molecumes of dry air in the same volume of air. See [this comment on mixing-ratio units](./general-information.html#units-and-abbreviations), item #4, for a description of how this variable is calculated from the measured concentration of water molecules and the atmospheric pressure and temperature.  
...
This is discussed in the memo available at [this link](www/ThetaE.pdf). The following summarizes that study. The Davies-Jones formula is:  
\begin{equation}
...
#### Virtual Temperature (ºC): TVIR {-#TVIR}
...
*A potential temperature analogous to the conventional potential temperature except that it is based on virtual temperature instead of ambient temperature.* Dry-adiabatic expansion or compression to the reference level (1000 hPa) is assumed. As for THETA, use of dry-air values for the gas constant and specific heat at constant pressure can lead to significant errors in humid conditions. For further information, see [this note](www/VirtualTemperature.pdf).<br />
...
RAF uses the radome<span id="radome gust-sensing system" label="radome gust-sensing system"></span> gust-sensing technique^[Brown, E. N, C. A. Friehe, and D. H. Lenschow, 1983: *Journal of Climate and Applied Meteorology,* **22,** 171–180]
...
See Eq.\ \@ref(eq:M2) for the equation used. The letter 'x' refers to the location, typically 'F' (fuselage) or 'R' (radome). The measurement from a special pitot-static sensor is denoted "FT" for fuselage-top.  Many relatively old archived data files have instead a variable XMACH2, which is the square of MACHx.  

#### Aircraft True Airspeed (m/s): TASx, TASX, TAS_A, TASDRY {-#true-airspeed}

*The flight speed of the aircraft relative to the atmosphere.* This derived measurement of the flight speed of the aircraft relative to the atmosphere is based on the Mach number calculated using the dynamic pressure at location x, the static pressure, and the measured water vapor pressure. See the derivation for ATx . The different variables for TASx (TASF, TASR, etc) use different measurements of QCxC in the calculation of Mach number. The variable TASDRY is the result of calculations for which the Mach number, air temperature, and true airspeed are determined for dry instead of humid air. See the discussion of [ATX](#ATX) for an explanation of how humidity is handled in the calculation of true airspeed. TASFLG is a data-quality flag indicating (by a value of 1 instead of normal 0) that the humidity correction or another input to TASX has a problem and has been flagged as unreliable. This often occurs because the humidity measured by a dew-point instrument is above the equilibrium value for the measured temperature. An alternate measure of true airspeed provided directly by the aircraft avionics system is TAS_A; this variable is apparently filtered in the avionic system and therefore is missing some of the high-frequency signal present in TASx from the research data system. The avionics system also provides a variable called CAS_A, which represents "Calibrated Airspeed". That variable applies calibrations to the "indicated airspeed," which is determined from the difference between dynamic pressure sensed at a pitot tube and static pressure sensed by static sources on the fuselage. Calibrated airspeed represents the flight speed that, for standard flight conditions of 1013.25 hPa and 15^o^C, would produce the measured pressure difference after calibration for measurement errors.    
...
calculation of TASX (documented above) now uses moist-air
...
#### Attack Angle Differential Pressure (hPa): ADIFR {-#adifr}

*The pressure difference between the bottom and top pressure ports of a radome gust-sensing system.* This measurement is used to determine the angle of attack; see AKRD below. Obsolete variable ADIF is a similar variable used for old gust-boom systems or for Rosemount Model 858AJ flow-angle sensors, and ADIFF has also been used in some archived projects.  

#### Sideslip Angle Differential Pressure (hPa): BDIFR {-#bdifr}
...
#### Attack Angle, Radome (º): AKRD {-#akrd}

*The angle of attack of the aircraft.* This derived measurement represents the angle between the longitudinal axis of the aircraft and the component of the relative wind vector in the plane of port-starboard symmetry of the aircraft. The tangent of the angle of attack is the ratio of the vertical to longitudinal component of the relative wind. Positive values indicate flow moving upward (in the aircraft reference frame) relative to the longitudinal axis. The calculation is based on ADIFR and a measurement of dynamic pressure. Empirical sensitivity coefficients for each aircraft, determined from special flight maneuvers, are used; see [RAF Bulletin 23](https://opensky.ucar.edu/islandora/object/archives%3A8152) and this [Technical Note](http://dx.doi.org/10.5065/D60G3HJ8) for more information. The sensitivity coefficients listed below have changed when the radomes were changed or refurbished, so the project documentation should be consulted for the values used in a particular project. For more information on the latest C-130 calibration, see [this note](www/ARISTO-LAMS.pdf).  
...
See also [this memo](www/AKRDC130.pdf).  


Beginning in 2017, a different strategy was used, as documented in more detail in [this memo](www/Recommendation161121.pdf). Two variables were used to develop empirical representations of the angle of attack, $A$={ADIFR}/{QCF} and $q$={QCF}. However, each was filtered into complementary low-pass and high-pass components, with the cutoff frequency at (1/600) Hz, and the separate components were used to represent the separate components of angle of attack according to the following formula:  

```{r child='newAKRDbox.Rmd'}
```


#### Reference Attack Angle (º): ATTACK {-#attack}
...
#### Sideslip Angle (Differential Pressure) (º): SSRD {-#ssrd}
...
#### Reference Sideslip Angle (º): SSLIP {-#sslip}
...
The wind vector discussed in this subsection is determined from the vector sum of the relative wind and the motion of the aircraft as determined from an inertial reference system. The wind components are determined in a reference frame call the "*l*" reference frame, a local cartesian system that rotates with the Earth and has positive values of the three components toward the east, north, and upward. This reference frame is described in detail in the technical note referenced below. Possible errors in the initial alignment of the inertial reference system, or similar errors that accumulate during flight, lead to errors in the reference system that oscillate with a Schuler oscillation. This is discussed in detail in [this NCAR Technical Note](Cooper, W. A. (2017). A Kalman Filter to Improve Measurements of Wind from NSF/NCAR Research Aircraft (No. NCAR/TN-540+STR). doi:10.5065/D61N7ZTS). The effects of the Schuler-oscillation errors can be minimized by employing one of these three correction schemes:  

1. The horizontal components of the wind can be improved by reference to measurements from a Global Positioning System unit, as described in Sect.\ \@ref(combining-irs-and-gps-measurements). This, however, does not lead to any improvement in the vertical wind.  
2. R routines are available that fit the Schuler oscillation to the errors between INS-provided vs.\ GPS-provided measurements of aircraft motion and use that fit to estimate the errors present in PITCH and ROLL. Two functions in the [Ranadu](http://doi.org/10.5281/zenodo.3830427) package for R, "CorrectPitch.R" and "CorrectRoll.R", can be used to determine these corrections.  
3. The full Kalman-filter approach documented in the above NCAR Technical Note can be used. Details regarding running this processor are in that Technical Note, or this processing can be added to standard archives upon request to [the RAF data-management group](mailto:raf-dm@eol.ucar.edu).

The last two methods can estimate the error present in PITCH and provide a correction. That improvement in the sensed orientation of the platform can lead to an improvement in the measured vertical wind by correcting for the error in pitch.  
...
*The horizontal wind components relative to the frame of reference attached to the aircraft.* UX is parallel to the longitudinal axis and positive toward the nose (i.e., if the wind direction is from behind the aircraft). VY is along the lateral axis and normal to the longitudinal axis; positive is toward the port (or left) wing.<br />  
...
#### Wind Vector, Vertical Component (m/s): WIC, WIY, WIX {-#wic}

*The component of the wind in the vertical direction.* This is the standard calculation of vertical wind, obtained from the difference between the measured vertical component of the relative wind and the vertical motion of the aircraft (usually GGVSPD in recent projects).`r ToDoLink('4.13')` This should be used in preference to WI even if the latter is present; see the discussion of WP3 in Section \@ref(inertial-reference-systems). Positive values are toward the zenith. Sometimes WIY, another calculation of vertical wind based on AKY instead of AKRD, is included, and WIX may indicate (in its attributes) which of WIC or WIY is thought to be better and will equal that preferred variable.  

#### GPS-Corrected Wind Speed and Direction (m/s and ^∘^): WSC, WDC {-#wsc-wdc}

*The magnitude and direction of the wind vector,* obtained by combining measurements from GPS and IRU units. These variables are obtained in a straightforward manner from UIC and VIC, using equations analogous to \@ref(eq:WS) and \@ref(eq:WD) but with UIC and VIC as input measurements. They are expected to be the preferred measurements of wind because they combine the best features of the IRU and GPS measurements.  
...
The above variables are normally included in the archived netCDF files from projects, but there are a few remote sensors that provide additional state-parameter measurements in some projects. These include:<span id="subsec:MTP" label="subsec:MTP"></span></p>
