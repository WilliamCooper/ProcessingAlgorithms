...
These variables were calculated from UI and VI, the east and north components of the wind determined as described in RAF Bulletin No. 23 and summarized in Section\ \@ref(wind):`r ToDoLink('10.1')`  
<p class=box2>
\begin{align}
\mathrm{\{WSPD\}} = & \sqrt{\mathrm{\{UI\}^{2}+\{VI\}^{2}}} (\#eq:WSobs)\\
\mathrm{\{WDRCTN\}} = & \frac{180^{\circ}}{\pi}\mathrm{atan2(\{UI\},\,\{VI\})}+180^{\circ}
(\#eq:WDobs)
\end{align}
</p>
...
The first two of these measurements were made directly in the chamber of the cryogenic hygrometer, a now obsolete cabin-mounted instrument connected to outside air by an inlet line. VCRH was determined from a third-order calibration equation applied to the voltage measured by the instrument. `r ToDoLink('10.3')`
...
CO is the uncorrected output of the TECO model 48 CO analyzer. `r ToDoLink('10.6')`This instrument measured the concentration of CO by gas filter correlation. The optics of the version operated by the RAF were modified to increase the light through the absorption cell, and a zero trap was added that periodically removed CO from the sample air stream to obtain an accurate zero. This permitted correction for the significant temperature-dependent drift of the zero level of the measurement.  
