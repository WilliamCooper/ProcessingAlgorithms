# General Information About Data Files {#general-information}
...
1. The millibar (mb), equal to one hectopascal (hPa), was used for pressure with some older variables. This report favors hPa, but pressure units in some archived data sets are still listed as mb.    
2. Many variables are presented in the units most often used for that variable, even when they involve CGS units or mixed CGS-MKS units, as for example [g m^-3^] for liquid water content or [cm^-3^] for droplet concentration.  
...
5. The unit “hertz” (abbreviation Hz) is the unit used here for a periodic sampling frequency, vs.\ the more awkward “samples per second.”  
...
SymbolTable <- data.frame("abbreviation-or-symbol"=c('º', 'ft', 'mb', 'ppmv', 'ppbv', 'pptv', 'n mi', 'kt'),
                          "definition$^a$" = c('degree, angle measurement ≡ $\\pi/180$', 'foot ≡ 0.3048 m',
                                         ...
                                         'knot (n mi/hour) ≡ (1852/3600) m/s = 0.514444… m/s$\\,^b$'))
...
knitr::kable(SymbolTable, align=c('cc')) %>% kable_styling(bootstrap_options = 'bordered') %>%
  add_footnote(c("where ≡ is used, the relation is exact by definition", "Until 2010, 0.514791 was used. This affected interpretation of all measurements from the INS and GPS instruments transmitted in units of knots."), notation="alphabet") %>%
  scroll_box(width='90%')
```
...
https://www.eol.ucar.edu/raf/Software/TimeExamp.html
