...
\newcommand{\IndexDef}[1]{*#1*}
\newcommand{\IndexPrimary}[1]{**#1**}
...
\vskip1inCurrent PDF version of this document: [Processing Algorithms](https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/ProcessingAlgorithms.pdf)\\
\vskip1inDownload this to a browser or pdf viewer for functional
URL links.
...
\clearpage\phantomsection{#sec:Symbols}
...
\hline 
**page** & **suggested action** & **who?**\tabularnewline
\hline 
\endhead
\hline 
...
document:
\begin{enumerate}
-  The reference version is ProcessingAlgorithms.lyx, which needs 'LyX',
...
-  The document generates three indices: a regular index, a list of symbols,
...
\begin{enumerate}
-  entries like 'wind!relative' will generate index entries as subordinate
...
coefficient.
-  It is sometimes useful to generate ``see xxx'' entries, which can
...
steps:
\begin{enumerate}
-  Export a LaTeX file from LyX in pdflatex format, using the ``export''
option.
-  Run ``pdflatex ProcessingAlgorithms'' three times to be sure all
references are resolved.
-  Run ``splitindex ProcessingAlgorithms'' to generate files for the
lists.
...
comment applies.
-  It is sometimes easiest to edit the PDF file directly. Some of the
