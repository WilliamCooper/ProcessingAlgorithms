...
```{r FIG1, fig.cap='Difference between spnerical geometry and cartesian geometry.'}
l2 <- 0
p2 <- 0
...
DF <- F1-FS
# cairo_pdf(filename='sphCoordFig1.pdf')
...
# invisible(dev.off())
# include_graphics('sphCoordFig1.pdf')
```
...
names(DT) <- c('AC lat', 'AC lon', 'REF lat', 'REF lon', 'sph D', 'cart D', 'sph A', 'cart A')
