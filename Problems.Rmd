...
<!-- Note: used rmarkdown::render("Problems.Rmd", output_format = 'pdf_document') to get pdf. -->
...
5. In the process, I also did lots of editing to remove remnant HTML code and make the document more consistent with RMarkdown. This mostly involved changing the reference and link style, recording tables to use kable instead of HTML, and removing lots of \<p\>-\</p\> tags while ending paragraphs with two spaces. I left some HTML, however, notably the box-paragraph styles used for algorithm boxes. There are probably some remnants I missed.  
6. I also edited Appendix C to update how the document should be edited.  
7. I then combined the list of problems below and the former "punch list", making it a separate document stored in the www directory so it could be edited easily and referenced apart from the TN.  I updated this list where possible.  
...
36. I spent a lot of time trying to get footnotes (that appear at the end of each section) to have superscript numbers, but the bookdown CSS specifies a normal number followed by a period. I learned that this is also the recommendation of the Chicago Manual of Style, so I finally gave up. There is probably a way to do this by modifying CSS.  
37. Regarding the VCSEL: CONCV_VXL indicates dependencies on PSXC, ATX, SMPLSRINT_VXL, MODE_VXL, and PREGAIN_VXL. This is unclear to me, and the use of these variables in the processing algorithm needs clarification. The basic measurement is RAWCONC_VXL, which is the sensed molecular density. This needs adjustment to ambient conditions, so the adjustment should at least reference the pressure and temperature in the sensed volume in addition to the ambient pressure and temperature, but those measurements are not included in the dependencies. How are the last three dependencies used. An updated processing algorithm is needed to clarify this.  
38. AT_VXL in netCDF files has units of "degK". The proper unit is "kelvin" (with lower-case k) or, abbreviated, "K".  
39.  