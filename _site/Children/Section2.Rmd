...
6. In some cases, particularly for older data files, speed has been recorded in units of knots (= 0.514444 m/s) and distance in nautical miles $\\equiv$ 1852 m).  
...
SymbolTable <- data.frame("abbreviation-or-symbol"=c('º', 'ft', 'mb', 'ppmv', 'ppbv', 'pptv', 'n mi', 'kt'),
                          "definition$^a$" = c('degree, angle measurement $\\equiv \\pi/180$', 'foot $\\equiv$ 0.3048 m',
                                         'millibar $\\equiv$ 100 Pa $\\equiv$ 1 hPa',
                                         ...
                                         'nautical mile $\\equiv$ 1852 m',
                                         'knot (n mi/hour) $\\equiv$ (1852/3600) m/s = 0.514444… m/s'))
...
knitr::kable(SymbolTable, align=c('cc')) %>% kable_styling(bootstrap_options = 'bordered') %>%
  add_footnote("'≡' indicates that the relation is exact by definition", notation="alphabet") %>%
  scroll_box(width='90%')
```
