...
and \href{https://opensky.ucar.edu/islandora/object/archives\%3A8152}{RAF Bulletin 23}.\index{Bulletin 23}
...
information on the coordinate system used, see \href{https://opensky.ucar.edu/islandora/object/archives\%3A8152}{RAF Bulletin 23}\index{Bulletin 23}.\label{punch3.2}
...
With INSs now in use, the internal drift that arises when this measurement
is integrated to get aircraft vertical speed and then altitude is
removed by the INS via pressure damping through reference to the pressure
altitude\index{pressure damping}.\footnote{For earlier projects using the Litton LTN-51 INS, this is a direct
...
\textbf{Aircraft Rate of Climb (m/s): }\textbf{\uline{ROC}}\index{rate of climb}\sindex[var]{ROC}\nop{VSPD}{}\textbf{
}(new 2017)\textbf{}\\
\emph{The rate of climb or upward speed of the aircraft, }as measured
by the INS with correction so as to represent the derivative of the
...
equation. The result retains the high-frequency response from the
INS while matching the low-frequency average value determined from
the hydrostatic equation, and so represents change in geometric height.
\href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/Recommendation161107f.pdf}{This memo}
contains additional background information on this variable.\\
\fbox{\begin{minipage}[t]{0.95\textwidth}%
$g$$^{\dagger}$ = acceleration of gravity (see Eq. \ref{eq:g_lambda})\\
...
\begin{enumerate}
\item From consecutive measurements of pressure, estimate the rate of climb
from the hydrostatic equation in the form:\\
\[
w_{p}=-\frac{R_{d}T_{k}}{gp}\frac{\Delta p}{\Delta t}
\]
\item Add the current measurement of acceleration to the cumulative sum:\\
\[
w_{p}^{*}\leftarrow w_{p}^{*}+a\Delta t
\]
\item Define ROC as the sum of $w_{p}^{*}$ and the low-pass filtered value
of ($w_{p}-w_{p}^{*}$):\\
\begin{align*}
...
relative to mean sea level, geopotential height and pressure height.
The WGS-84 height (measured by GPS instruments) is height relative
to a reference system in which zero is defined by a specified reference
ellipsoid representing the shape of the Earth, as specified here:
\href{http://earth-info.nga.mil/GandG/publications/tr8350.2/tr8350_2.html}{http://earth-info.nga.mil/GandG/publications/tr8350.2/tr8350\_2.html},.
This is not defined to be a level surface in the sense of being an
equipotential surface. A better approximation of mean sea level is
the ``geoid'', a surface having constant gravitational equipotential
that approximates that at mean sea level. The geoid is more structured
than the WSR-84 reference ellipsoid and departs significantly from
it, often by several 10s of meters. Even the geoid does not represent
mean sea level exactly because local mean sea level can be influenced
by variations in water density, mean wind, or ocean circulation, but
the geoid is usually the reference used for measurements labeled ``MSL''
except when fine-scale local effects must be considered. The geometric
height is the true height above a reference surface, often taken to
be mean sea level or the geoid; this may therefore differ significantly
from the height measured directly by a GPS unit. There is a variable
included below, GGEOIDHT, that provides a measure of the difference.
Geopotential height is the height above mean sea level that would
give the geopotential, or gravitational potential energy per unit
mass, of the actual parcel if that mass were raised against standard
...
in January 2014 new logs (named BESTPOS and BESTVEL) have also been
recorded to preserve more significant digits in the measurements.
The BESTPOS log has a position resolution of $10^{-11}\deg$, while
the BESTVEL log is recorded with 0.0001 m/s resolution. 
...
rotation is therefore applied to GGVSPD, as described below.
\begin{hangparagraphs}
...
originates from an avionics-system GPS. In the case of GGVEW, when
...
from $^{\circ}\thinspace s^{-1}$ to radians~$s^{-1}$. This correction
is not applied in normal processing because the use of the complementary
filter, discussed in Sect.\_\ref{subsec:IRS/GPS}, makes it of negligible
importance. More information is contained in \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/Recommendation161107.pdf}{this memo}.
...
from $^{\circ}\thinspace s^{-1}$ to radians~$s^{-1}$. This correction
is not applied in normal processing because the use of the complementary
filter, discussed in Sect.~\ref{subsec:IRS/GPS}, makes it of negligible
importance, as discussed in the note referenced for GGVEW.

\textbf{GPS-Measured Aircraft Vertical Velocity (m/s):}\nop{GGVSPD}{}\textbf{
...
(obsolete)\\
\emph{The aircraft vertical velocity provided by a GPS unit.} \index{velocity!aircraft vertical}Positive
values are upward.\label{punch3.9} When GGVSPD is used in the calculation
of vertical wind, the following correction\index{correction!vertical wind!rotation in pitch}
(omitted before 2017) is applied:\label{punch:3-13}\\
\[
...
archives. See \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/Recommendation161107.pdf}{this memo}
for additional justification.

\textbf{GPS Altitude (m MSL): }\textbf{\uline{GGALT}}\sindex[var]{GGALT}\index{GGALT},
...
the geoid as represented internally by the GPS receiver and is determined
by adding the adjustment $-$GGEOIDHT to the direct measurement relative
to the ellipsoidal Earth model of the GPS, which is defined by WGS-84.\index{World Geodetic System}\index{WGS84}
Positive values are above the reference surface. GGALT originates
from a data-system GPS; GALT\_A originates from an avionics-system
GPS. See the discussion of height at the beginning of this subsection
and the variable GGEOIDHT below for interpretation of these GPS-based
measurements. 

\textbf{GPS Altitude above the WGS94 reference ellipse (m):}\nop{GGHMSL}\textbf{\uline{
GGHWGS}}\index{GGHWGS}\sindex[var]{GGHWGS}\\
\emph{The aircraft altitude above the ellipse that is the reference
for the WGS-84 coordinate system. }This variable is defined as GGHWGS=GGALT$+$GGEOIDHT.\label{punch3.4-1}

\textbf{GPS Geopotential Altitude (m):}\nop{GEOPTH} \textbf{\uline{GEOPTH}}\sindex[var]{GEOPHT}\index{GEOPHT}\\
\emph{The aircraft geopotential altitude above mean sea level.} If
$g(z,\lambda)$ is the acceleration of gravity as represented by the
formula in the Table of Constants on page \pageref{constants-table},
then the formula used for calculation of GEOPTH is obtained by integrating
that formula from the reference surface for MSL (the geoid, $\Delta$
above the WGS84 reference ellipse, to the geometric altitude $H$,
which is $H+\Delta$ above the reference ellipse. The result is normally
close (within about 0.5~m) to that obtained with $\Delta=0$. There
are additional details in \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/RecommendationReGEOPHT.pdf}{this memo}.\\
...
\textbf{GPS Height of the Geoid (m):}\nop{GGEOIDHT}{}\textbf{ }\textbf{\uline{GGEOIDHT\sindex[var]{GGEOIDHT}\index{GGEOIDHT}}}\\
\emph{Height of geoid, approximating mean sea level, above the WGS-84
ellipsoid.} The height above mean sea level is found by subtracting
this value from the height above the WGS-84 reference ellipse as provided
by GPS-based measurements.\label{punch3.4}
...
\textbf{Height Above Terrain (m):}\nop{ALTG}{} \textbf{\uline{ALTG}}\index{ALTG}\sindex[var]{ALTG}\\
\emph{The aircraft altitude above the Earth's surface} as represented
by the next variable. If GGALT is the altitude above mean sea level,
ALTG=GGALT$-$SFC.

\textbf{Height of the Earth's Surface (m MSL):}\nop{SFC}{} \textbf{\uline{SFC}}\index{SFC}\sindex[var]{SFC}\\
\emph{The altitude of the Earth's surface} at a location directly
below the aircraft. The data source is the Shuttle Radar Topography
Mission of 2000. The height estimate is described in \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/HeightOfTerrain.pdf}{this memo}.
...
pressure (PSXC).}\index{altitude!pressure}\index{International Standard Atmosphere}
...
about 8--40 m over the altitude range of the GV.} A note at \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/PressureAltitude.pdf}{this link}
...
-0.0065 $^{\circ}$C per geopotential meter = the lapse rate\index{International Standard Atmosphere!lapse rate}
...
a complementary-filter\index{filter!complementary (for wind)} algorithm.\label{compFilter}
...
Filtering and Optimum Linear Estimation, }p. 49. As described in \href{https://github.com/NCAR/aircraft_ProcessingAlgorithms/blob/master/www/checkCompFilter.pdf}{this memo},
...
ground speed\\
\hyperlink{GGVNS}{GGVNS}\index{GGVNS} = GPS-measured north component
of the aircraft ground speed\\
